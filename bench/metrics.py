...
def time_to_lock(result: Result, scenario: Scenario) -> float | None:
    """Return the first hardware lock, or an analog settling time if absent."""
...
def false_trigger_count(result: Result, scenario: Scenario) -> int:
    ...
    locked = time_to_lock(result, scenario) is not None
