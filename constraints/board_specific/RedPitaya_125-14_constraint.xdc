#
# RedPitaya_125-14.xdc
#
# Red Pitaya STEMlab 125-14 physical constraints (ADC, DAC, LED, PWM DAC,
# XADC, expansion connector, SATA/daisy, and the associated clock
# constraints).
#
# THIS FILE IS NOT NEW WORK. It is Red Pitaya's own official constraints
# file (original @Id red_pitaya.xdc 961 2014-01-21, @Author Matej Oblak,
# (c) Red Pitaya d.o.o., BSD-licensed -- Red Pitaya's LICENSE file covers
# the fpga/ directory recursively under a 3-clause BSD license). Sourced
# here via the PyRPL project (pyrpl-fpga/pyrpl, an established open-source
# Red Pitaya lockbox), which redistributes it unmodified.
#
# It is included verbatim rather than re-derived, deliberately: pin
# location constraints for a 125 MSPS ADC/DAC interface are exactly the
# kind of detail where a transcription error is invisible until it
# corrupts data or, worse, damages hardware. Do not hand-edit PACKAGE_PIN
# values here from memory or a datasheet re-read -- if a pin ever needs to
# change, re-derive it from an authoritative Red Pitaya source, not by
# guessing.
#
# posm_lock_core (this project's Red Pitaya wrapper) does NOT need its own
# separate constraints: it has no top-level package pins of its own. It is
# instantiated as a leaf module inside red_pitaya_top.v/.sv and reached
# only through internal wires (adc_a/adc_b, dac_a/dac_b, sys_* bus) that
# are already downstream of the pin-constrained ports below. This file is
# what makes those top-level ports legal, which is a precondition for
# building at all, not something specific to this project's addition.
#
############################################################################
# IO constraints                                                          #
############################################################################
...
### LED
# LED[3:0] carry posm_lock_core's o_lock_state, LED[4] carries
# o_lock_fault, LED[5] carries o_trace_ready -- see the LED wiring
# comment in red_pitaya_top.v where posm_lock_core is instantiated.
# LED[7:6] are free for bring-up use.
...
# posm_lock_core and its sys_* register bus run entirely within the
# adc_clk domain (same clock as red_pitaya_top.v's existing adc_a/adc_b/
# dac_a/dac_b path) -- no new clock domain crossing is introduced by this
# project's addition, so no new timing exceptions are needed beyond what's
# already above.