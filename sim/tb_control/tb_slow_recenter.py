...
def main():
    ...
    def bench(ctx):
        ...
        # the test.
        yield from bus_write(ctx, dut, ADDR_SLOW_RECENTER_TARGET, 0)
        yield from bus_write(ctx, dut, ADDR_SLOW_RECENTER_GAIN, int(0.25 * (1 << GAIN_FRAC)))
        yield from bus_write(ctx, dut, ADDR_SLOW_OUT_MIN, (-32768) & 0xFFFF)
        yield from bus_write(ctx, dut, ADDR_SLOW_OUT_MAX, 32767)
        yield from bus_write(ctx, dut, ADDR_SLOW_SLEW_LIMIT, 5000)
        yield from bus_write(ctx, dut, ADDR_SLOW_CTRL_CONFIG,
