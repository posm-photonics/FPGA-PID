...
async def bus_read(ctx, dut, addr):
    ctx.set(dut.adr, addr)
    ctx.set(dut.we, 0)
    ctx.set(dut.stb, 1)
    await ctx.tick()
...
def main():
    ...
    async def bench(ctx):
        await bus_write(ctx, dut, ADDR_TRACE_CONFIG, 1 << TRACE_CFG_ENABLE)
        await bus_write(ctx, dut, ADDR_TRACE_LENGTH, N)
        await bus_write(ctx, dut, ADDR_TRACE_DECIM, 1)
        await bus_write(ctx, dut, ADDR_TRACE_START, 1)
        ...
        await ctx.tick()
        status = await bus_read(ctx, dut, ADDR_TRACE_STATUS)
        assert status & (1 << TRACE_STAT_READY), f"expected ready, got status={status:#x}"
 
        await bus_write(ctx, dut, ADDR_TRACE_READ_ADDR, 3)
        await ctx.tick()
        x = await bus_read(ctx, dut, ADDR_TRACE_READ_DATA_X)
        y = await bus_read(ctx, dut, ADDR_TRACE_READ_DATA_Y)
